...
def _call_doubao_api(url, api_key, prompt, max_retries):
    """豆包API (火山方舟) 调用"""
    headers = {
    ...
    payload = {
        "model": "Doubao-pro-32k",  # 使用豆包专业版模型
        "messages": [
            {
                "role": "user",
                "content": prompt
    ...
    for attempt in range(max_retries + 1):
        try:
            print(f"【豆包API】第 {attempt + 1}/{max_retries + 1} 次尝试...")
...
def _call_zhipu_api(url, api_key, prompt, max_retries):
    ...
    payload = {
        "model": "glm-4-flash",  # 使用永久免费模型（新用户送 2000 万 Token）
        "messages": [
            {
                "role": "user",
                "content": prompt
    ...
    for attempt in range(max_retries + 1):
        try:
            print(f"【智谱 AI】第 {attempt + 1}/{max_retries + 1} 次尝试...")
            response = requests.post(url, headers=headers, json=payload, timeout=API_TIMEOUT)
            response.raise_for_status()
...
def _call_huggingface_api(url, api_key, prompt, max_retries):
    """Hugging Face API 调用"""
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json"
    ...
    for attempt in range(max_retries + 1):
        try:
            print(f"【Hugging Face】第 {attempt + 1}/{max_retries + 1} 次尝试...")
            response = requests.post(url, headers=headers, json=payload, timeout=API_TIMEOUT)
            response.raise_for_status()
            
            result = response.json()
            
            # 解析 Hugging Face API 返回的内容
            ...
            last_error = "API 请求超时"
            print(f"【Hugging Face】⚠️ 超时，已重试 {attempt} 次")
            time.sleep(1)
            
        except requests.exceptions.HTTPError as e:
            status_code = e.response.status_code if hasattr(e, 'response') else 'Unknown'
            last_error = f"HTTP 错误 {status_code}: {str(e)}"
            print(f"【Hugging Face】❌ HTTP 错误：{status_code}")
            if status_code == 410:
                print("【Hugging Face】⚠️ 模型端点不可用 (410 Gone)")
                break
            time.sleep(1)
            
        except Exception as e:
            last_error = f"未知错误：{str(e)}"
            print(f"【Hugging Face】❌ 异常：{e}")
            break
    
    return {
        'success': False,
        'content': None,
        'error': last_error or 'Hugging Face 调用失败'
    }
...
def call_ai_api(prompt, api_url=None, token=None, max_retries=None, api_type="auto"):
    ...
    Args:
        ...
        api_type: API 类型 ("auto"|"zhipu"|"huggingface")
    ...
    if api_type == "auto":
        ...
        if DOUBAO_API_KEY:
            api_list.append({"type": "doubao", "url": DOUBAO_API_URL, "key": DOUBAO_API_KEY})
        # Hugging Face 总是尝试（作为备用）
        api_list.append({"type": "huggingface", "url": HF_API_URL, "key": HF_TOKEN})
    else:
        # 指定 API 模式
    ...
    for api_info in api_list:
        ...
        try:
            if api_name == "zhipu":
                ...
                result = _call_doubao_api(current_url, current_key, prompt, max_retries)
            elif api_name == "huggingface":
                result = _call_huggingface_api(current_url, current_key, prompt, max_retries)
            else:
                continue
...
"""
    ...
    try:
        headers = {
            "Authorization": f"Bearer {ZHIPU_API_KEY_TEXT}",
            "Content-Type": "application/json"
        }
...
class FoodGuardianAI(ctk.CTk):
    ...
    def open_voice_interaction(self):
        ...
        input_frame = ctk.CTkFrame(dialog, fg_color=COLORS['card_bg'], corner_radius=15)
        ...
        voice_input_display = ctk.CTkTextbox(input_frame, height=220, font=("Arial", 18))
        voice_input_display.pack(padx=15, pady=(0, 15), fill=tk.X)
        ...
        output_frame = ctk.CTkFrame(dialog, fg_color=COLORS['card_bg'], corner_radius=15)
        ...
        def set_display(text):
            ...
            voice_input_display.insert(tk.END, text)

        def show_confirm_actions():
            if not confirm_frame.winfo_ismapped():
                confirm_frame.pack(pady=10)

        def hide_confirm_actions():
            if confirm_frame.winfo_ismapped():
                confirm_frame.pack_forget()

        def reset_record_button(text="?? ????", enabled=True):
            if dialog.winfo_exists():
        ...
        def reset_voice_state(message=None):
            state['recognized_text'] = ''
            state['stop_requested'] = False
            state['is_recording'] = False
            state['is_asking_ai'] = False
            hide_confirm_actions()
            stop_btn.pack_forget()
            stop_btn.configure(text="?? ??????", state='normal')
            reset_record_button("?? ????")
            set_status("????")
            if message is None:
                message = (
        ...
        def get_speakable_text():
            if state['latest_answer']:
                return state['latest_answer']
        ...
        def speak_text_async(text_to_speak):
            ...
            def worker():
                try:
                    ...
                    safe_after(lambda: speak_btn.configure(text="?? ???...", state='disabled'))
                    safe_after(lambda: set_status("??????"))
                    ...
                    safe_after(lambda: speak_btn.configure(text="?? ??????", state='normal'))
                    safe_after(lambda: set_status("????"))
                except ImportError:
                    safe_after(lambda: messagebox.showerror("??", "??? pyttsx3 ?\n\n????pip install pyttsx3"))
                    safe_after(lambda: speak_btn.configure(text="?? ??????", state='normal'))
                    safe_after(lambda: set_status("??????"))
                except Exception as e:
                    safe_after(lambda e=e: messagebox.showerror("??", f"???????{e}"))
                    safe_after(lambda: speak_btn.configure(text="?? ??????", state='normal'))
                    safe_after(lambda: set_status("??????"))
                finally:
        ...
        def speak_current_text():
            text_to_speak = get_speakable_text()
            if not text_to_speak:
                messagebox.showwarning("??", "?????????????????????????")
                return
            speak_text_async(text_to_speak)

        def stop_recording():
            if not state['is_recording']:
                return
            state['stop_requested'] = True
            stop_btn.configure(text="? ????...", state='disabled')
            set_status("????????")
            set_display("? ????????????...\n\n??????????????")

        def handle_recording_error(display_text, status_text, dialog_text):
            state['is_recording'] = False
            state['stop_requested'] = False
            stop_btn.pack_forget()
            stop_btn.configure(text="?? ??????", state='normal')
            ...
            if dialog_text:
                messagebox.showerror("????", dialog_text)

        def on_recognition_success(text):
            state['is_recording'] = False
            state['stop_requested'] = False
            state['recognized_text'] = text
            stop_btn.pack_forget()
            stop_btn.configure(text="?? ??????", state='normal')
            show_confirm_actions()
            reset_record_button("?? ????")
            set_status("?????????")
            set_display(f"?? ?????\n{text}\n\n??????? AI??????????")

        def start_listening():
            if state['is_recording'] or state['is_asking_ai']:
                return
            ...
            hide_confirm_actions()
            record_btn.configure(text="?? ???...", fg_color='red', state='disabled')
            stop_btn.configure(text="?? ??????", state='normal')
            ...
            def worker():
                try:
                    ...
                    if "PyAudio" in error_msg:
                        ...
                        dialog_text = "??? SpeechRecognition ?\n\n????pip install SpeechRecognition"
                    else:
                        dialog_text = f"???????{error_msg}"
                    safe_after(lambda: handle_recording_error("? ????????????", "??????", dialog_text))
                    return
                ...
                def record_with_sounddevice():
                    ...
                    with sd.InputStream(samplerate=sample_rate, channels=1, dtype='int16') as stream:
                        ...
                        while time.time() - start_time < 20:
                            ...
                            if overflowed:
                                print("[语音识别] sounddevice 缓冲区溢出，继续录音")
                    ...
                    if not chunks or not heard_voice:
                        raise sr.WaitTimeoutError("未检测到有效语音")
                ...
                try:
                    ...
                    except (AttributeError, OSError) as mic_error:
                        print(f"[语音识别] 默认麦克风录音不可用，切换到 sounddevice：{mic_error}")
                        safe_after(lambda: set_status("正在使用备用录音方式"))
                        safe_after(lambda: set_display("🎙️ 默认录音方式不可用，正在切换到备用录音方式，请继续说话..."))
                        audio = record_with_sounddevice()

                    if audio is None:
                        safe_after(lambda: reset_voice_state("⏹️ 本次录音已取消。\n\n点击下方按钮重新开始。"))
                        return

                    if state['stop_requested']:
                        safe_after(lambda: reset_voice_state("?? ????????\n\n???????????"))
                        return

                    safe_after(lambda: set_status("??????"))
                    safe_after(lambda: set_display("?? ????????..."))
                    ...
                    safe_after(lambda text=text: on_recognition_success(text))
                except sr.WaitTimeoutError:
                    safe_after(lambda: handle_recording_error(
                        "? ???????????",
                        "????",
                        "??????????\n\n??????????????????"
                    ))
                    ...
                    safe_after(lambda e=e: handle_recording_error(
                        f"? ?????{e}",
                        "???????",
                        f"???????????{e}\n\n?????????"
                    ))
                    ...
                    safe_after(lambda e=e: handle_recording_error(
                        f"? ???????{e}",
                        "??????",
                        f"备用录音方式缺少依赖：{e}\n\n请运行：pip install sounddevice numpy"
                    ))
                except Exception as e:
                    safe_after(lambda e=e: handle_recording_error(
                        f"? ???????{e}",
        ...
        def edit_text():
            ...
            ctk.CTkLabel(edit_dialog, text="?? ??????", font=("Arial", 18, "bold")).pack(pady=15)

            edit_textbox = ctk.CTkTextbox(edit_dialog, height=200)
            edit_textbox.pack(padx=20, pady=10, fill=tk.BOTH, expand=True)
            edit_textbox.insert(tk.END, state['recognized_text'])
            ...
            def save_edit():
                ...
                if not new_text:
                    messagebox.showwarning("??", "???????")
                    return
                state['recognized_text'] = new_text
                set_display(f"?? ??????????\n{new_text}\n\n????????????? AI?")
                set_status("???????")
                edit_dialog.destroy()
            ...
            ctk.CTkButton(btn_frame_edit, text="?? ??", command=save_edit, fg_color=COLORS['primary']).pack(side=tk.LEFT, padx=5)
            ctk.CTkButton(btn_frame_edit, text="? ??", command=edit_dialog.destroy, fg_color="transparent", border_width=2).pack(side=tk.LEFT, padx=5)

        def cancel_and_retry():
            reset_voice_state("??????????\n\n?????????????")

        def confirm_and_ask_ai():
            if not state['recognized_text']:
                messagebox.showwarning("??", "???????????")
                return
            if state['is_asking_ai']:
                return

            state['is_asking_ai'] = True
            ...
            def worker():
                try:
                    ai_prompt = (
                        "??????????????????????????????"
                        "??????????????????????\n\n"
                        f"{state['recognized_text']}"
                    )
                    api_result = call_ai_api(ai_prompt, api_type="auto")

                    if api_result.get('success'):
                        ai_answer = api_result.get('content', '').strip()
                        if not ai_answer:
                            raise ValueError("AI ??????")

                        def on_success():
                            state['latest_answer'] = ai_answer
                            set_display(f"? AI ???\n\n{ai_answer}")
                            set_status("AI ??????????")
                        ...
                        safe_after(on_success)
                    else:
                        error_msg = api_result.get('error', 'AI ????')

                        def on_error():
                            set_display(f"? AI ?????{error_msg}")
                            set_status("AI ????")
                            messagebox.showerror("??", f"AI ?????{error_msg}")
                            confirm_btn.configure(state='normal')
                            edit_btn.configure(state='normal')
                            cancel_btn.configure(state='normal')
                            record_btn.configure(state='normal')
    ...
    def _do_recommend(self, user_intake, population_group, fridge_items):
        ...
        def call_ai_async():
            try:
                ...
                def show_error():
                    self.recommendation_output.delete(1.0, tk.END)
                    self.recommendation_output.insert(tk.END, f"❌ 生成推荐失败：{str(e)}\n\n请检查网络连接或稍后重试。")
                    if hasattr(self, 'generate_rec_btn'):
                        self.generate_rec_btn.configure(state='normal', text="🍽️ 生成推荐")
...
【智能菜品生成规则】
1. **根据食材数量智能决定菜品数量**：
   - 如果用户输入的食材种类≤3 种，可以只生成 1 个菜
   - 如果用户输入的食材种类>3 种，必须生成多个菜品（2-4 个菜），合理分配食材到不同菜品中
...
（如有更多菜品，继续按此格式列出）"""
                    ...
                    def show_error():
                        if hasattr(self, 'menu_output'):
                            ...
                            self.menu_output.insert(tk.END, f"❌ 生成菜单失败：{str(e)}\n\n请检查网络连接或稍后重试。")
                        if hasattr(self, 'generate_menu_btn'):
                            self.generate_menu_btn.configure(state='normal', text="🍽️ 生成菜单")
...
环保意义：xxx"""
                ...
                def show_error():
                    if hasattr(self, 'scrap_output'):
                        self.scrap_output.delete(1.0, tk.END)
                        self.scrap_output.insert(tk.END, f"❌ 生成失败：{str(e)}")
                self.after(0, show_error)
...
请用清晰格式呈现，数据要科学合理。"""
                ...
                def show_error():
                    self.nutri_output.delete(1.0, tk.END)
                    self.nutri_output.insert(tk.END, f"❌ 发生错误：{str(e)}")
                self.after(0, show_error)
...
请用清晰的表格或列表格式呈现，方便用户在超市购物时使用。"""
                ...
                def show_error():
                    self.shopping_output.delete(1.0, tk.END)
                    self.shopping_output.insert(tk.END, f"❌ 发生错误：{str(e)}")
                self.after(0, show_error)
